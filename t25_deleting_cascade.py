...
# remove one Address 
jack_address = session.query(Address).get(1)
session.delete(jack_address)

# Count the number of email address for jack, only one address remains
jack_email = session.query(Address).filter(Address.email_address.in_(['jack@google.com', 'j25@yahoo.com'])).count()
print(jack_email)
...
# Count the number of User with name 'jack' 
jack_email = session.query(User).filter_by(name='jack').count()
print(jack_email)
